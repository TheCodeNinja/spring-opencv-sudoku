...
public final class EvalUtil {
    ...
    private static final ImagePreProcessingScaler IMAGE_PRE_PROCESSING_SCALER =
    ...
    public static int evaluateImage(final Mat image, final MultiLayerNetwork model) throws IOException {
        try (final INDArray input = Nd4j.create(1, HEIGHT * WIDTH)) {
            ...
            final INDArray predicted = model.output(dataSet.get(0).getFeatures(), false);

            final INDArray predictedValue =
                    BooleanIndexing.firstIndex(predicted, Conditions.equals(predicted.maxNumber()));
